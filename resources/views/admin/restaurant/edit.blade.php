...
<div class="py-12">
  <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
    ...
    <x-partials.card>
      ...
      <form method="POST" action="{{ route('restaurant.update', $restaurant->id) }}" class="mt-4"
       ...
       <div class="flex flex-wrap">
         ...
         <div class="my-2 w-full">
           ...
           <select name="restaurant_user" id="restaurant_user"
             ...
             <option value="{{ $restaurant_user->id }}">{{ $restaurant_user->name }}</option>
             @endforeach
         ...
         <div class="my-2 w-full">
           ...
           <input type="text" id="name" name="name" value="{{ $restaurant->name }}"
             ...
             maxlength="255" placeholder="Name" required="required" autocomplete="off">
           @error('name')
           <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
           @enderror
         </div>
         ...
         <div class="my-2 w-full">
           ...
           <input type="text" id="phone" name="phone" value="{{ $restaurant->phone }}"
             class="w-full px-3 py-2 text-gray-700 bg-gray-100 rounded-md border border-stroke focus-visible:outline-none @error('phone') border-red-500 @enderror"
             maxlength="20" placeholder="Phone" autocomplete="off">
           @error('phone')
           <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
           @enderror
         </div>
         ...
         <div class="my-2 w-full">
           ...
           <input type="text" id="address" name="address" value="{{ $restaurant->address }}"
             class="w-full px-3 py-2 text-gray-700 bg-gray-100 rounded-md border border-stroke focus-visible:outline-none @error('address') border-red-500 @enderror"
             maxlength="255" placeholder="Address" autocomplete="off">
           @error('address')
           <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
           @enderror
         </div>
         ...
         <div class="my-2 w-full">
           ...
           <input type="time" id="start_time" name="start_time" value="{{ $restaurant->start_time ? \Carbon\Carbon::parse($restaurant->start_time)->format('H:i') : '' }}"
             class="w-full px-3 py-2 text-gray-700 bg-gray-100 rounded-md border border-stroke focus-visible:outline-none @error('start_time') border-red-500 @enderror"
             autocomplete="off">
           @error('start_time')
           <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
           @enderror
         ...
         <div class="my-2 w-full">
           ...
           <input type="time" id="end_time" name="end_time" value="{{ $restaurant->start_time ? \Carbon\Carbon::parse($restaurant->end_time)->format('H:i') : '' }}"
             class="w-full px-3 py-2 text-gray-700 bg-gray-100 rounded-md border border-stroke focus-visible:outline-none @error('end_time') border-red-500 @enderror"
             autocomplete="off">
           @error('end_time')
           <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
           @enderror
         ...
         <div class="my-2 w-1/2 pr-3">
           <label for="logo" class="label font-medium text-gray-700 block mb-2"> Logo <small>(Ratio 1:1 || Max:
               500kb)</small></label>
           <div class="drop-area" id="drop-area-logo">
             <input type="file" id="file-input-logo" name="logo" accept="image/*" hidden="">
             <img id="preview-logo" src="{{ $restaurant->logo ? asset('storage/' . $restaurant->logo) : '' }}"
               alt="Logo Preview" style="display: {{ $restaurant->logo ? 'block' : 'none' }}; max-width: 100%;">
         ...
         <div class="my-2 w-1/2">
           <label for="banner" class="label font-medium text-gray-700 block mb-2"> Banner <small>(Ratio 3:1 || Max:
               1mb)</small></label>
           <div class="drop-area" id="drop-area-banner">
             <input type="file" id="file-input-banner" name="banner" accept="image/*" hidden="">
             <img id="preview-banner" src="{{ $restaurant->banner ? asset('storage/' . $restaurant->banner) : '' }}"
               alt="Banner Preview" style="display: {{ $restaurant->banner ? 'block' : 'none' }}; max-width: 100%;">
