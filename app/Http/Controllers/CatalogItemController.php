    ...
    try {
      ...
      $validatedData = $request->validate(
        [
          ...
          'product' => 'required|image|mimes:jpeg,png,jpg,gif,webp|max:2048', // Validate the product image
        ],
        [
      ...
      $productImagePath = $request->file('product')
        ? $request->file('product')->store('restaurant/product', 'public')
        : null;
    ...
    try {
      ...
      if ($request->hasFile('product')) {
        ...
        if ($catalogItem->product) {
          Storage::disk('public')->delete('restaurant/product/' . $catalogItem->product);
        }

        // Store the new image
        $productImagePath = $request->file('product')->store('restaurant/product', 'public');
        $validatedData['image'] = $productImagePath; // Store only the filename
      }
